...
// reduce() to combine together all the items in a collection using sum function
...
val divideThree = numlist.filter( (x:Int) => x % 3 != 0)
val dividedThree= numlist.map( (x:Int) => {x % 3 == 0 })
